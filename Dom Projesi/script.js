
var items=['item 1','item 2','item 3','item 4'];

var list=document.querySelector("#mylist");


items.forEach(function(item){
   
    CreateItem(item);
});



list.addEventListener("click",function(item){

    if(item.target.tagName="LI"){
        item.target.classList.toggle("checked");
        ToogleDeleteButton();

    }
});
function ToogleDeleteButton(){
    var checkList=document.querySelectorAll(".list-group-item.checked");
    if (checkList.length>0){
        document.querySelector("#deleteAll").classList.remove("d-none");  //itemları seçtiğimiz zaman d-none kalkacak ve Delete All sağ üstte görünecek
    }
    else{
        document.querySelector("#deleteAll").classList.add("d-none");

    }
}
document.querySelector("#deleteAll").addEventListener("click",function(){

    var elements=document.querySelectorAll(".checked");

    elements.forEach(function(item){
        item.style.display="none";

    });

});

document.querySelector("#btnCreate").addEventListener("click",function(){

    var item=document.querySelector("#txtItem").value ;
    // console.log(item);

    if(item===""){
        alert("lütfen bir değer giriniz");
        return;
    }
    CreateItem(item);

});

function CreateItem(item){
    var li=document.createElement("li");
    var t=document.createTextNode(item);
     li.className="list-group-item";
     li.appendChild(t);

     list.appendChild(li);

     var span=document.createElement("span");
     var text=document.createTextNode("\u00D7");
     span.className="close";
     span.appendChild(text);
     li.appendChild(span);
     var close=document.getElementsByClassName("close");
     for(var i=0; i<close.length; i++)
       close[i].onclick=function(){
       var li=this.parentElement;
       li.style.display="none";
       li.classList.remove("checked");   //sildiğimiz item ları spandan da sildik
        }

     
    }


  